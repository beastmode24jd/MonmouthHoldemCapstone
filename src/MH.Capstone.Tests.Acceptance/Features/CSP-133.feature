...
Scenario: Password Reset Email Request
  ...
  When they submit a password reset request with email "lily@test.com"
  Then they see a confirmation message that does not reveal whether the email is registered

Scenario: Reset Link Validity
  Given a valid password reset link has been generated for "lily@test.com"
...
Scenario: Successful Password Reset
  ...
  And the user can log in with email "lily@test.com" and password "CSP133Reset1!"
  And the acceptance database is restored to its original seed state

Scenario: Invalid or Expired Reset Link
  Given the user navigates to a reset link with an invalid token for "lily@test.com"
...
Scenario: Multiple Reset Requests Invalidate Previous Tokens
  Given a first password reset link has been generated for "alex@test.com"
  And a second password reset link has been generated for "alex@test.com"
  When the user navigates to the first reset link and submits password "CSP133AlexReset1!"
  Then the user sees an invalid reset link error message
