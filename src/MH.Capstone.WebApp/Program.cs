...
namespace MH.Capstone.WebApp
{
    public class Program
    {
        ...
        public static WebApplication Configure(WebApplicationBuilder builder, ILoggerFactory? startupLoggerFactory = null)
        {
            ...
            // Acceptance tests run over plain HTTP to avoid dev-certificate requirements.
            // HTTPS redirection would cause every request to redirect to an HTTPS endpoint
            // that isn't listening, making the app unreachable.
            if (!app.Environment.IsEnvironment("Acceptance"))
            {
                app.UseHttpsRedirection();
            }
            app.UseStaticFiles();
...
// Exposes Program to the integration test project so WebApplicationFactory<Program> can access it.
>>>>>>> e55db4c (Fix acceptance test in-process WebApp hosting; Add WebApp log output; Configure exisiting feature/tests)
